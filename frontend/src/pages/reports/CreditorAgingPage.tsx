...
export default function CreditorAgingPage() {
  ...
  const { data, isLoading, isFetching } = useQuery<AgingData>({
    queryKey: ['creditor-aging', queryParams],
    queryFn: () => get('/reports/ap/vendor-aging', queryParams!),
    enabled: !!queryParams,
  });
  ...
  return (
    <div className="space-y-6 print:space-y-4">
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Creditor Aging</h1>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-16 text-gray-500 dark:text-gray-400">Click <span className="font-semibold text-indigo-600">Generate</span> to view creditor aging.</div>
        ) : data.rows.length === 0 ? (
          <div className="text-center py-16 text-gray-500 dark:text-gray-400">No outstanding creditor balances found.</div>
        ) : (
            ...
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-700/50">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Vendor Name</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-700/50">
                {data.rows.map((row) => (
                  <tr key={row.vendorCode} className="hover:bg-gray-50 dark:hover:bg-slate-700/30">
                    <td className="px-4 py-2 text-sm font-mono text-gray-600 dark:text-gray-400">{row.vendorCode}</td>
                    <td className="px-4 py-2 text-sm text-gray-900 dark:text-white">{row.vendorName}</td>
